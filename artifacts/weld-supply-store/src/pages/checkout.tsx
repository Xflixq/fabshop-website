...
export default function Checkout() {
  ...
  const onSubmit = (values: CheckoutFormValues) => {
    createOrder.mutate(
      ...
      {
        onSuccess: (order) => {
          // Clear cart local state/invalidate
          queryClient.invalidateQueries({ queryKey: getGetCartQueryKey({ sessionId }) });
          // Generate new session ID for future orders so the old cart is gone
          localStorage.setItem("weldforge_session_id", crypto.randomUUID());
          
          toast({
            title: "Order confirmed!",
            description: "Your gear is being prepped for shipment.",
          });
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="john@example.com" type="email" className="h-12 font-mono" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="font-bold uppercase text-xs tracking-wider text-muted-foreground">Complete Shipping Address</FormLabel>
                        <FormControl>
                          <Input placeholder="123 Workshop Ln, Industrial Park, CA 90210" className="h-12 font-mono" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <div className="pt-6 border-t">
                    ...
                    <p className="text-center text-xs text-muted-foreground font-mono mt-4 flex items-center justify-center">
                      <ShieldCheck className="h-4 w-4 mr-1 text-primary" /> Secure encrypted transaction
                    </p>
                  </div>
                </form>
              </Form>
